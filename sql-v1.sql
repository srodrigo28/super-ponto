...
-- Criação da função calculate_hours
CREATE OR REPLACE FUNCTION public.calculate_hours()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  ...
  -- Calcular horas extras assumindo 8 horas como jornada padrão
  IF NEW.total_horas IS NOT NULL THEN
    NEW.hora_extra := NEW.total_horas - interval '8 hours';
    IF NEW.hora_extra < interval '0 hours' THEN
      NEW.hora_extra := interval '0 hours';
...
-- Criação do gatilho calculate_hours_trigger
CREATE TRIGGER calculate_hours_trigger
BEFORE INSERT OR UPDATE ON public.registro
FOR EACH ROW
EXECUTE FUNCTION public.calculate_hours();

-- Consulta para trazer registros com o nome do funcionário
SELECT 
  registro.id,
  registro.created_at,
...
FROM 
  public.registro
JOIN 
  public.funcionario ON registro.user_ref = funcionario.id
LIMIT 5;
